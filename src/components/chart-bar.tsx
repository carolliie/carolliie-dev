...
export function ChartBarHorizontal() {
  ...
  const total = React.useMemo(() => ({
    desktop: Array.isArray(chartData) && chartData.length > 0
      ? chartData.reduce((acc, curr) => acc + (curr.desktop || 0), 0)
      : 0,
    mobile: Array.isArray(chartData) && chartData.length > 0
      ? chartData.reduce((acc, curr) => acc + (curr.mobile || 0), 0)
      : 0,
  }), [chartData]);
